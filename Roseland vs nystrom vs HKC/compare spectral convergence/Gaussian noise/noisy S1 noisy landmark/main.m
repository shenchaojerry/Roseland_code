...
for K = 1:iter
    ...
    %% HKC
    u_Haddad = HKC(data, num_eigvec, ref, 1, sig);
...
%% compare first two eigenvec closely
...
angle_nys = angle(v1 + 1i * v2) + pi;
angle_ref = angle(v3 + 1i * v4) + pi;
angle_Haddad = angle(v5 + 1i * v6) + pi;
theta = theta(pick);

% phase recovered by nystrom
figure('Renderer', 'painters', 'Position', [10 10 1000 900]);
scatter(theta, angle_nys, 30, 'filled'); axis tight;
...
% phase recovered by Roseland
figure('Renderer', 'painters', 'Position', [10 10 1000 900]);
scatter(theta, angle_ref, 30, 'filled'); axis tight;
xticks([0.01 pi/2 pi 3*pi/2 2*pi-0.01])
xticklabels({'0','\pi/2','\pi','3\pi/2','2\pi'})
yticks([0.01 pi/2 pi 3*pi/2 2*pi-0.01])
yticklabels({'0','\pi/2','\pi','3\pi/2','2\pi'})
...
% phase recovered by HKC
figure('Renderer', 'painters', 'Position', [10 10 1000 900]);
scatter(theta, angle_Haddad, 30, 'filled'); axis tight;
xticks([0.01 pi/2 pi 3*pi/2 2*pi-0.01])
xticklabels({'0','\pi/2','\pi','3\pi/2','2\pi'})
yticks([0.01 pi/2 pi 3*pi/2 2*pi-0.01])
yticklabels({'0','\pi/2','\pi','3\pi/2','2\pi'})
...
% compare amplitude recovery by nystrom and Roseland
figure('Renderer', 'painters', 'Position', [10 10 1200 800]); hold on; 
ylim([0.0105,0.0172]); xlim([1, 9000])
plot(sqrt(v1.^2 + v2.^2), 'linewidth', 3); 
plot(sqrt(v3.^2 + v4.^2), 'linewidth', 3); 
set(gca,'xtick',[])
xlabel('Angel \theta', 'fontsize', 45)
set(gca, 'FontSize', 30)
[~, hobj, ~, ~] = legend({'Nystrom','Roseland'},'Fontsize',35);
hl = findobj(hobj,'type','line');
set(hl,'LineWidth',6);

% compare amplitude recovery by HKC and Roseland
figure('Renderer', 'painters', 'Position', [10 10 1200 800]); hold on; 
ylim([0.0105,0.0172]); xlim([1, 9000])
plot(sqrt(v5.^2 + v6.^2), 'linewidth', 3); axis tight;
plot(sqrt(v3.^2 + v4.^2), 'linewidth', 3); 
set(gca,'xtick',[])
xlabel('Angel \theta', 'fontsize', 45)
set(gca, 'FontSize', 30)
[~, hobj, ~, ~] = legend({'HKC','Roseland'},'Fontsize',35);
hl = findobj(hobj,'type','line');
set(hl,'LineWidth',6);
